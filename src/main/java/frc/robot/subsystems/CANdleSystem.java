...
public class CANdleSystem extends SubsystemBase {
    ...
    private final int LedCount = 300;//灯珠数量，设的比较小，有可能灯带只亮一半

    private Animation m_toAnimate = null;//要变换到的状态
                                         //一种灯效 → 一种状态
                                         //灯效的切换 → 不同状态的切换

    public enum AnimationTypes { //包含所有要枚举的变量 （this is a 属性）
        ColorFlow,
        Fire,
        Larson,
        Rainbow,
    ...
    public CANdleSystem() {   //构造函数 特点：1.名字和类保持一致 2.无返回类型声明
        //this.joystick = joy;
    ...
    //通常，我们采用标志位
    //flag
    //int flag 当flag为1时候，表示当前在状态1
    //int flag 当flag为2时候，表示当前在状态2
    //int next_flag 用来表示下一个我要切换到的状态

    public void incrementAnimation() { //方法  //状态递增  //从状态1变到下一个状态
    ...
    //ColorFlow → Fire → Larson → Rainbow ... ...(形成一个圈，一大堆后又回到colorflow)
    ...
    public void setOff() {
        ...
        // m_candle.animate(null);
        // m_candle.setLEDs(0, 0, 0);     //红色+蓝色=紫色
                                          //像素值最高255，r=255  纯红色
    ...
    public Command setColorFlowWithMotor() {
    ...
    //有一个比较炫酷的灯效，我想让他一直保持整场比赛，我该怎么写
    //periodic():每20ms就调用一次，那我只需要把.animate写在periodic()
